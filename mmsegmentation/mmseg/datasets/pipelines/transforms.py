...
class CropROI(object):
    ...
    def __call__(self, results):
        """Call function to crop ROI using connection information
        maps.
        ...
        Returns:
            dict: crop results, 'crop_index' keys is added into
                result dict.
        """
        bbox = self.connect_analysis(results['img'])
        results["bbox"] = bbox
        results["res_shape"] = results["ori_shape"]
        results["ori_shape"] = (bbox[2] - bbox[0],bbox[3] - bbox[1],results["ori_shape"][2])
        results["img"] = results["img"][bbox[0]:bbox[2],bbox[1]:bbox[3],:]
        
        return results


    def connect_analysis(self,pred):
        pred = cv2.cvtColor(pred,cv2.COLOR_BGR2GRAY)
        _,pred = cv2.threshold(pred,5,255,cv2.THRESH_BINARY)

        label = measure.label(pred)
        region_props = measure.regionprops(label)
        length = len(region_props)
        max_area = region_props[0].area
        index = 0
